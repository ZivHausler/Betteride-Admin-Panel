...
const Log = ({ time, type, text, server, index }) => {
    return (
        <div onClick={() => alert(`time: ${time}\nserver: ${server}\ntype: ${type}\ntext: ${text}`)}
            ...
            <div className='w-2.5 h-2.5 rounded-full shadow-sm mr-1 '>
                <div className={`w-2.5 h-2.5 rounded-full bg-${type === "OK" ? "green" : type === "ERROR" ? "red" : 'yellow'}-400 shadow-sm mr-1 animate-pulse`}></div>
            </div>
